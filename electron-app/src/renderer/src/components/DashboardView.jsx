...
export default function DashboardView({ timerState }) {
  ...
  const load = useCallback(async () => {
    ...
    try {
      const result = await window.electronAPI.invoke('focusblocks:get', localDateStr());
      setBlocks(result || []);
    } catch (e) {
  ...
  useEffect(() => {
    if (!window.electronAPI) return;
    const unsub = window.electronAPI.on('timer:tick', () => load());
    return unsub;
  }, [load]);
  ...
  const expectedBreaks = Math.max(0, Math.floor(focusHours * 2));
  const breakDeficit = Math.max(0, expectedBreaks - breakCount);
  const waterGoal = 8;
  const eyeStrainIndex = Math.min(100, Math.round((focusSec / 3600) * 20));
      ...
      <div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
          <motion.div className={card} {...fadeUp} transition={{ delay: 0.2 }}>
            ...
            <ProgressRing progress={waterCount / waterGoal} size={72} strokeWidth={5} color="hsl(var(--primary))">
